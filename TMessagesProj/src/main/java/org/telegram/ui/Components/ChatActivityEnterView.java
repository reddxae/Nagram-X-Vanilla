                    ...
                    if (alpha < 0.45) {
                        ...
                        if (recordAudioVideoRunnableStarted) {
                            ...
                            if (hasRecordVideo && isInVideoMode()) {
                                ...
                                CameraController.getInstance().cancelOnInitRunnable(onFinishInitCameraRunnable);
                                delegate.needStartRecordVideo(NekoConfig.confirmAVMessage.Bool() ? 3 : 1, true, 0, voiceOnce ? 0x7FFFFFFF : 0, effectId, 0);
                                sendButton.setEffect(effectId = 0);
                                ...
                                if (!NekoConfig.confirmAVMessage.Bool()) {
                                    if (recordingAudioVideo && isInScheduleMode()) {
                                        AlertsCreator.createScheduleDatePickerDialog(parentActivity, parentFragment.getDialogId(), (notify, scheduleDate) -> MediaController.getInstance().stopRecording(1, notify, scheduleDate, voiceOnce, 0), () -> MediaController.getInstance().stopRecording(0, false, 0, voiceOnce, 0), resourcesProvider);
                                    }
                                }
                                delegate.needStartRecordAudio(0);
                                if (!NekoConfig.confirmAVMessage.Bool()) {
        ...
        if (messageEditText != null && messageEditText.getText().length() > 0) {
            if (containsMarkdown(messageEditText.getText())) {
                ...
                options.add(markdownButtonDrawable, markdownButtonStr, () -> {
                    sentFromPreview = System.currentTimeMillis();
                    sendMessageInternal(!NaConfig.INSTANCE.getSilentMessageByDefault().Bool(), 0, 0, true, SendMessageInternalParams.markdown(withoutMarkdown));
                    if (!containsSendMessage && messageSendPreview != null) {
                        messageSendPreview.dismiss(true);
                        messageSendPreview = null;
                    } else {
                ...
                options.add(R.drawable.casino_icon, getString(R.string.SendAsEmoji), () -> {
                    sentFromPreview = System.currentTimeMillis();
                    sendMessageInternal(!NaConfig.INSTANCE.getSilentMessageByDefault().Bool(), 0, 0, true, SendMessageInternalParams.game(false));
                    if (!containsSendMessage && messageSendPreview != null) {
                        messageSendPreview.dismiss(true);
                        messageSendPreview = null;
                    } else {
                ...
                options.add(markdownButtonDrawable, markdownButtonStr, () -> {
                    sentFromPreview = System.currentTimeMillis();
                    sendMessageInternal(!NaConfig.INSTANCE.getSilentMessageByDefault().Bool(), 0, 0, true, SendMessageInternalParams.pangu(shouldUsePangu));
                    if (!containsSendMessage && messageSendPreview != null) {
                        messageSendPreview.dismiss(true);
                        messageSendPreview = null;
                    } else {
...
//        }
    ...
    private boolean sendMessage() {
        if (isInScheduleMode()) {
            ...
            return sendMessageInternal(!NaConfig.INSTANCE.getSilentMessageByDefault().Bool(), 0, 0, true);
        }
    }
    ...
    protected boolean sendMessageInternal(boolean notify, int scheduleDate, long payStars, boolean allowConfirm, SendMessageInternalParams internalParams) {
        final Runnable send = () -> {
    ...
    public boolean processSendingText(CharSequence text, boolean notify, int scheduleDate, long payStars, SendMessageInternalParams internalParams) {
